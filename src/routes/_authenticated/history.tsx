...
export const Route = createFileRoute('/_authenticated/history')({
  head: () => meta('Riwayat Transaksi', 'Pantau seluruh permintaan deposit dan penarikan akun Anda.'),
  component: Page,
})
...
function Page() {
  ...
  const shown = useMemo(() => (tab === 'Semua' ? items : items.filter((i) => i.kind === tab)), [items, tab])

  const masuk = items.filter((i) => i.kind === 'Deposit' && i.status === 'approved').reduce((s, i) => s + i.amount, 0)
  const keluar = items.filter((i) => i.kind === 'Penarikan' && i.status === 'approved').reduce((s, i) => s + i.amount, 0)
  ...
  return (
    <AppShell back="/profile" title="RIWAYAT TRANSAKSI">
      ...
      <Stat label={`Selisih Pemasukan & Pengeluaran • ${items.length} transaksi`} value={rupiah(masuk - keluar)} accent />
      <div className="mt-2 grid grid-cols-2 gap-2">
        <Stat label="Total Deposit Disetujui" value={rupiah(masuk)} />
        <Stat label="Total Penarikan Disetujui" value={rupiah(keluar)} />
      </div>
      <p className="mt-2 text-[10px] text-muted-foreground">Akumulasi seluruh riwayat, bukan saldo tersedia.</p>
      ...
      <div className="space-y-2">
        {shown.map((i) => (
          <Card key={`${i.kind}-${i.id}`}>
            ...
            {i.note && <p className="mt-2 text-[11px] text-muted-foreground">Catatan admin: {i.note}</p>}
            {i.proof && (
              <div className="mt-3">
        ...
        {!loading && shown.length === 0 && (
          <Empty title="Belum Ada Transaksi" text="Riwayat deposit dan penarikan Anda akan muncul di halaman ini." />
        )}
      </div>
