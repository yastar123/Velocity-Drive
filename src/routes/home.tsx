...
import { AppShell, Card, CopyButton, Notice, PageIntro, QuickActions, SectionTitle, Stat } from '@/components/menara-ui'
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [{ data: profile }, { data: deps }, { count }] = await Promise.all([
        ...
        supabase.from('deposit_requests').select('amount').eq('user_id', auth.user.id).eq('status', 'approved'),
        supabase
          .from('orders')
  ...
  return (
    <AppShell>
      ...
      <PageIntro eyebrow="DASBOR INVESTOR" title={text['home_hero_title'] ?? 'Selamat datang kembali'}>
      ...
      <QuickActions />
      <SectionTitle aside={<span className="text-xs text-primary">4 lantai</span>}>Lantai Investasi</SectionTitle>
      ...
      <Card>
        ...
        <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
          {text['about_text'] ?? 'Model investasi berbasis lantai dengan informasi performa yang transparan.'}
        </p>
      ...
      <p className="mt-6 text-center text-[10px] text-muted-foreground">© 2026 Menara Miliarder</p>
    </AppShell>
  )
}
